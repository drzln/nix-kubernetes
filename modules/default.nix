...
{
  lib,
  pkgs, # node-side nixpkgs
  config,
  inputs, # exposed through specialArgs
  ...
}:
##############################################################################
# Local helpers / assertions
##############################################################################
let
  inherit
    (lib)
  ...
  _ =
    lib.assertMsg (lib.isFunction baseOverlay)
    "blackmatter.components.kubernetes.overlay must be an overlay "
    + "function (`self: super: { … }`).";
  ...
  # ────────── Package resolver (fail fast) ────────────────────────────────
  require = name:
    if pkgs' ? ${name}
    then pkgs'.${name}
    else
      builtins.throw ''
        Package “${name}” not found in your overlay!
        The BlackMatter Kubernetes module is **self-contained** and requires
        every Kubernetes-related package to come from your flake.
      '';
  ...
  etcdPkg =
    if cfg.etcdPackage != null
    then cfg.etcdPackage
    else require "etcd";
  containerdPkg =
    if cfg.containerdPackage != null
    then cfg.containerdPackage
    else require "containerd";
  ...
  # Render kubeadm.yml from attrset
